...
df = add_twd97_km_columns(df)

X_raw = df[["x_km", "y_km"]].to_numpy(dtype=np.float64)
...
out = add_twd97_km_columns(
    pd.DataFrame(grid_raw, columns=["lon", "lat"])
)
X, grid, _ = center_train_test_coordinates(
    X_raw,
    out[["x_km", "y_km"]].to_numpy(dtype=np.float64),
)
