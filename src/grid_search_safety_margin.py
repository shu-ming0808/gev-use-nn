...
def metric_rows(true_params, pred_params, method, safety_margin):
    rows = []
...
def constraint_summary(params, extrema, method, safety_margin):
...
def run_grid_search(margins, penalty_weight=0.1, epochs=150, patience=8, seed=111):
    ...
    for margin in margins:
        ...
        print("=" * 72)
        print(f"Training safety_margin={margin:.4f}")
        train_constraint_penalty(
            seed=seed,
        ...
        baseline_metric = pd.DataFrame(
            metric_rows(true_params, baseline_params, "Baseline", margin)
        )
        constraint_metric = pd.DataFrame(
            metric_rows(true_params, constraint_params, "Constraint penalty", margin)
        )
        all_metrics.append(baseline_metric)
        all_metrics.append(constraint_metric)

        baseline_summary = constraint_summary(
            baseline_params, extrema_valid, "Baseline", safety_margin=margin
        )
        constraint_summary_row = constraint_summary(
            constraint_params, extrema_valid, "Constraint penalty", safety_margin=margin
        )
        constraint_score = score_metrics(constraint_metric)
        xi_rmse = constraint_metric.loc[constraint_metric["param"] == "xi", "RMSE"].iloc[0]
        ...
        print(
            f"m={margin:.4f} | score={constraint_score:.6f} | "
    ...
    best_row = constraint_summary_df.sort_values(["rmse_score", "violation_rate"]).iloc[0]

    print("=" * 72)
    print("Best safety margin by total RMSE score:")
    print(best_row)
